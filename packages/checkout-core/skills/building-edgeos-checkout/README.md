...
Documents the **0.2.0** surface. Since 0.1.0: no `flowSlug` anywhere (the client
resolves the popup's primary flow itself), `getRuntime()` replaced by
`getProducts()` + `getForm()`, and no steps or step navigation, since screens are
the client app's own state.
...
| `api-contract.md` | Heavy reference: every endpoint + response field, money/status semantics, error codes, recipients for ticket lines, what a publishable key relaxes, the buyer base/`custom_` field split. | Everyone (framework-agnostic) |
