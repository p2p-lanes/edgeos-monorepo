 ...
 *   - A product with category="other", max_quantity=1, is_active=true
...
function makeProduct(overrides: Partial<ProductsPass> = {}): ProductsPass {
  return {
    ...
    price: 25,
    is_active: true,
    max_quantity: 1, // single-shot Add/Added toggle (supportsQuantitySelector → false)
    description: null,
    compare_price: null,
    original_price: null,
    duration_type: null,
...
describe("VariantMerchImage — category='other' bug reproducer (unit-level)", () => {
  ...
  it("renders the Add button for a product with category='other' and max_quantity=1", () => {
    render(
      <VariantMerchImage
        products={[product]}
        stepType="merch"
    ...
    // max_quantity=1 → supportsQuantitySelector returns false → Add/Added toggle button
