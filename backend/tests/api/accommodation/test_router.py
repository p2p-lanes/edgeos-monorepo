...
class TestBookingsAndCalendar:
    ...
    def test_calendar_reports_no_availability_outside_the_bookable_window(
        self, client: TestClient, admin_token_tenant_a: str, popup_tenant_a: Popups
    ) -> None:
        """An empty unit on a night nobody may book is not availability.

        The calendar greys those nights, and the number under them has to
        agree: an operator who sees "2 free" will sell a stay the checkout
        then refuses.
        """
        prop = _create_property(client, admin_token_tenant_a, popup_tenant_a.id)
        _create_accommodation(
            ...
            prop["id"],
            units_count=2,
            bookable_from="2026-06-05",
            bookable_to="2026-06-08",
        )
        ...
        room = response.json()["properties"][0]["accommodations"][0]
        assert room["bookable_from"] == "2026-06-05"
        assert room["bookable_to"] == "2026-06-08"
        assert room["is_active"] is True

        availability = room["availability_by_day"]
        assert availability["2026-06-04"] == 0
        assert availability["2026-06-05"] == 2
        # bookable_to is a check-out bound: the 7th is the last night on sale.
        assert availability["2026-06-07"] == 2
        assert availability["2026-06-08"] == 0

    def test_calendar_reports_no_availability_for_a_room_type_switched_off(
        self, client: TestClient, admin_token_tenant_a: str, popup_tenant_a: Popups
    ) -> None:
        prop = _create_property(client, admin_token_tenant_a, popup_tenant_a.id)
        accommodation = _create_accommodation(
        ...
        assert set(room["availability_by_day"].values()) == {0}
