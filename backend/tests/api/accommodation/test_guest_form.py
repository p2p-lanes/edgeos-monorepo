...
pinning is the precedence between the step and the property, and the exact
order the answers are checked in, since the portal mirrors both and a
divergence means a buyer fixes one refusal and walks into another.
"""
...
def _property(mode: str = "inherit", form: dict | None = None) -> SimpleNamespace:
    return SimpleNamespace(guest_form_mode=mode, guest_form=form)
...
class TestResolveForm:
    def test_a_property_inherits_the_step_form_by_default(self) -> None:
        form = resolve_form(STEP_FORM, _property())

        assert form is not None
        assert [field.key for field in form.booker.fields] == ["full_name", "email"]

    def test_off_asks_nothing_even_when_the_step_asks_plenty(self) -> None:
        assert resolve_form(STEP_FORM, _property("off")) is None

    def test_custom_replaces_the_step_form_rather_than_adding_to_it(self) -> None:
        own = {"booker": {"fields": [AGE]}}

        form = resolve_form(STEP_FORM, _property("custom", own))

        assert form is not None
        assert [field.key for field in form.booker.fields] == ["age"]

    def test_custom_with_no_form_of_its_own_asks_nothing(self) -> None:
        # Not "falls back to the step": the operator said this property is
        # different, and quietly re-inheriting would contradict them.
        assert resolve_form(STEP_FORM, _property("custom", None)) is None

    def test_a_step_with_no_form_asks_nothing(self) -> None:
        assert resolve_form(None, _property()) is None

    def test_a_form_with_no_fields_is_no_form(self) -> None:
        assert resolve_form({"booker": {"fields": []}}, _property()) is None
