...
Schema only: the invites and referrals tables are created empty, with no
...
def downgrade() -> None:
    """Reverse the groups-rework migration by dropping all new schema objects.

    upgrade() creates no data, so there is nothing to reverse before dropping
    the schema. Any data written after the migration is dropped with the tables;
    back up or clean up manually first if needed.
    """
    # ------------------------------------------------------------------
