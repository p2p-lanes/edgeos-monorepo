...
The questions belong to the checkout, not to the inventory: they live on the
accommodation step, in ``template_config.guest_form``, so an operator writes
them once and every room that step offers asks the same thing. Letting a
single property ask something of its own is an additive extension and is
deliberately not here yet: one form is what almost every gathering wants, and
two would be a precedence rule to explain before a single question is asked.
