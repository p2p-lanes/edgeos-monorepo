/**
 * The questions a property asks about the people staying.
 *
...
import type { AccommodationGuestForm } from "@/client"
...
/** How a property answers "are the step's questions asked here". */
export type GuestFormMode = "inherit" | "off" | "custom"
...
/** A form that asks nothing is no form: it is stored as null, so a property
 *  reading it inherits or asks nothing rather than rendering an empty panel. */
...
): AccommodationGuestForm | null {
  if (isEmpty(form) || !form) return null
  return form as unknown as AccommodationGuestForm
}
