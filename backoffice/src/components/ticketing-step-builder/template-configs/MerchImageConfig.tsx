...
const MERCH_VARIANTS = [
  {
    value: "default",
    label: "Default",
    description: "List rows with responsive mobile/desktop layout",
  },
  {
    value: "compact",
    label: "Compact",
    description: "Minimal rows with small image thumbnails",
  },
  {
    value: "grid",
    label: "Grid",
    description: "2-column cards with large product images",
  },
] as const
      ...
      {[0, 1].map((i) => (
        ...
        >
          <div className="flex items-center gap-0.5 shrink-0">
            <div className="w-1 h-1 rounded-sm bg-muted-foreground/15" />
  ...
  return (
    <div className="grid grid-cols-2 gap-1 w-full">
      {[0, 1].map((i) => (
        ...
        >
          <div className="h-6 bg-muted-foreground/10" />
          <div className="p-0.5 flex flex-col gap-0.5">
            <div className="h-0.5 w-6 rounded-full bg-muted-foreground/20" />
            <div className="flex items-center justify-between">
              <div className="h-0.5 w-3 rounded-full bg-muted-foreground/15" />
              <div className="h-1 w-4 rounded-full bg-muted-foreground/10" />
            </div>
          </div>
        </div>
      ))}
...
export function MerchImageConfig({ config, onChange }: TemplateConfigProps) {
    ...
    <div className="flex flex-col gap-3">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Choose how merchandise is displayed in the checkout
        </p>
      </div>
      <div className="grid grid-cols-3 gap-2">
        {MERCH_VARIANTS.map((v) => {
