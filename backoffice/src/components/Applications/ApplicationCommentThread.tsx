...
import { type ApplicationCommentPublic, ApplicationsService } from "@/client"
...
function commentsQueryKey(applicationId: string) {
  return ["application-comments", applicationId]
}
...
function CommentRow({
  ...
  comment: ApplicationCommentPublic
}) {
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({
      queryKey: commentsQueryKey(applicationId),
    })

  const updateMutation = useMutation({
    mutationFn: () =>
      ApplicationsService.updateApplicationComment({
        applicationId,
  ...
  const deleteMutation = useMutation({
    mutationFn: () =>
      ApplicationsService.deleteApplicationComment({
        applicationId,
        commentId: comment.id,
      }),
    onSuccess: () => {
...
export function ApplicationCommentThread({
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      ApplicationsService.createApplicationComment({
        applicationId,
        requestBody: { body },
      }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-sm text-muted-foreground">
        Notes that justify this application's rating. Visible to the team.
      </p>
      ...
      {comments.length === 0 ? (
        ...
        <div className="space-y-2">
          {comments.map((c) => (
            <CommentRow key={c.id} applicationId={applicationId} comment={c} />
          ))}
        </div>
      )}
